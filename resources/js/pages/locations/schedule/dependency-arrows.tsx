...
export default function DependencyArrows({ links, visibleTasks, rangeStart, dayWidth, showBaseline, tempLink, onClickLink }: DependencyArrowsProps) {
    const taskMap = new Map<number, TaskNode>();

    // Compute cumulative Y positions accounting for variable row heights
    const taskYCenter = new Map<number, number>();
    let cumulativeY = 0;
    for (const t of visibleTasks) {
        taskMap.set(t.id, t);
        const hasBaseline = showBaseline && t.baseline_start && t.baseline_finish;
        const rowH = hasBaseline ? ROW_HEIGHT + 16 : ROW_HEIGHT;
        // Anchor to the actual bar center (top portion when baseline visible)
        const barCenterY = hasBaseline ? cumulativeY + 4 + ROW_HEIGHT / 2 : cumulativeY + rowH / 2;
        taskYCenter.set(t.id, barCenterY);
        cumulativeY += rowH;
    }
    ...
    return (
        <svg className="pointer-events-none absolute inset-0 z-10 text-gray-600 dark:text-gray-300" style={{ width: '100%', height: totalHeight }}>
            ...
            {links.map((link) => {
                ...
                const path = buildSmoothPath(x1, srcY, x2, tgtY, srcPoint, tgtPoint);
...
function buildSmoothPath(
    x1: number, y1: number,
    x2: number, y2: number,
    srcPoint: 'start' | 'finish',
    tgtPoint: 'start' | 'finish',
): string {
