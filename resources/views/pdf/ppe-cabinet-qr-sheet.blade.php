...
<head>
    <meta charset="UTF-8" />
    <title>PPE/RPE Register — {{ $location->name ?? 'Cabinet' }}</title>
    <style>
...
<div class="page">
    ...
    @endif
    <div class="eyebrow">PPE / RPE Register</div>
    <h1>Scan to record PPE issued</h1>
    <p class="meta">
        @if($location) <strong>{{ $location->name }}</strong> @endif
        @if($location && $location->external_id) · Job {{ $location->external_id }} @endif
    </p>
    ...
    <div class="steps">
        ...
        <div class="step">
            <div class="num">3</div>
            <div class="title">Record items &amp; submit</div>
            <div class="body">Select PPE collected, quantity and supervisor.</div>
        </div>
    </div>
