...
if (process.argv[1] && resolve(process.argv[1]) === resolve(fileURLToPath(import.meta.url))) {
  ...
  if (result.deletedPaths.length === 0 && result.removedLockEntries.length === 0) {
    writeLine('No deprecated Impeccable skills found. Nothing to clean up.');
  } else {
    if (result.deletedPaths.length > 0) {
      writeLine(`Removed ${result.deletedPaths.length} deprecated skill(s):`);
      for (const p of result.deletedPaths) writeLine(`  - ${p}`);
    }
    if (result.removedLockEntries.length > 0) {
      writeLine(`Cleaned ${result.removedLockEntries.length} entry/entries from skills-lock.json:`);
      for (const name of result.removedLockEntries) writeLine(`  - ${name}`);
    }
  }
}
